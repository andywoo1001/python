...
Ui_MainWindow, QMainWindow = loadUiType('window_v7.ui')
...
class Main(QMainWindow, Ui_MainWindow):
	def __init__(self, ):
		...
		self.actionStart.triggered.connect(self.StartButton)
		self.actionStop.triggered.connect(self.StopButton)
		#self.dial_Little.mousePressEvent(self.DialLittlePress)
		#self.dial_Little.mouseReleaseEvent(self.DialLittleRelease)

		MainWindow = self

		self.showfreq = ShowFreq()
		self.showfreq_value = int(10**(13.0/10.0)) #self.SlideBar.getValue()
		...
		#self.ax = [ self.fig.add_subplot(811), self.fig.add_subplot(812), self.fig.add_subplot(813), self.fig.add_subplot(814),
		#			self.fig.add_subplot(815), self.fig.add_subplot(816), self.fig.add_subplot(817), self.fig.add_subplot(818)]
		...
		line12,= self.ax[8].plot([], [], linestyle='-', color='#00FFFF', linewidth=width, antialiased=anti) # FPS
		
		self.lines = [line0,line1,line2,line3,line4,line5,line6,line7,line8,line9,line10,line11,line12]
		#self.lines = [line0,line1,line2,line3,line4,line5,line6,line7,line8,line9,line10,line11]
		...
		self.ax[8].yaxis.set_ticks(numpy.arange(0, 71, 10)) # FPS
	...
	def update_lines(self, data):
		...
		self.LCD_FPS.display('%.1f' % values[24])
		...
		if (self.enable_display == True): 
			...
			self.lines[12].set_ydata(self.mFPS[-100:]     + padd)			
		else:
			for line in self.lines:
				line.set_ydata([None]*100)
	...
	def DVFS_Table(self):
		...
		if(DEVICE_CHIP == 'universal7420'):		#N5
			print ('universal7420')
			CPU0tbl = Bash(ADB + 'shell cat /sys/devices/system/cpu/cpufreq/mp-cpufreq/cluster0_freq_table')
			CPU1tbl = Bash(ADB + 'shell cat /sys/devices/system/cpu/cpufreq/mp-cpufreq/cluster1_freq_table')
			GPUtbl  = Bash(ADB + 'shell cat /sys/devices/14ac0000.mali/dvfs_table')
		elif (DEVICE_CHIP == 'universal3475'):	#J2
			print ('universal3475')
			CPU0tbl = Bash(ADB + 'shell cat /sys/devices/system/cpu/cpufreq/cpufreq_table')
			CPU1tbl = Bash(ADB + 'shell cat /sys/devices/system/cpu/cpufreq/cpufreq_table')
			GPUtbl  = Bash(ADB + 'shell cat /sys/devices/11400000.mali/dvfs_table')
		else:
		...
		self.Little_Min = str(int(CPU0tbl[len(CPU0tbl)-1])/1000)
		self.Little_Max = str(int(CPU0tbl[0])/1000)

		self.Big_Min = str(int(CPU1tbl[len(CPU1tbl)-1])/1000)
		self.Big_Max = str(int(CPU1tbl[0])/1000)
		
		self.GPU_Min = GPUtbl[len(GPUtbl)-1]
		self.GPU_Max = GPUtbl[0]
		...
		# QDial Value
		self.dial_Little.setValue( (int(self.Little_Min)+int(self.Little_Max))/2 )
		self.dial_Big.setValue( (int(self.Big_Min)+int(self.Big_Max))/2 )
		self.dial_GPU.setValue( (int(self.GPU_Min)+int(self.GPU_Max))/2 )

		# QDial Text
		self.label_little_min.setText(self.Little_Min) 
		self.label_little_max.setText(self.Little_Max) 
		self.label_big_min.setText(self.Big_Min)
		self.label_big_max.setText(self.Big_Max) 
		self.label_gpu_min.setText(self.GPU_Min) 
		self.label_gpu_max.setText(self.GPU_Max)

		# QDial Ranges
		self.dial_Little.setMinimum(int(self.Little_Min))
		self.dial_Little.setMaximum(int(self.Little_Max))
		self.dial_Big.setMinimum(int(self.Big_Min))
		self.dial_Big.setMaximum(int(self.Big_Max))
		self.dial_GPU.setMinimum(int(self.GPU_Min))
		self.dial_GPU.setMaximum(int(self.GPU_Max))
	...
	def UpdateShowFreq(self, value):		
		self.showfreq_value = int(10**(float(value)/10.0))
	...
	def ActionPerformanceMode(self):
		...
		if(DEVICE_CHIP == 'universal7420'):		#N5
			...
			"""system(ADB + 'shell \"chmod 777 /sys/devices/platform/gpusysfs/gpu_min_clock\"'  )
			system(ADB + 'shell \"echo %d > /sys/devices/platform/gpusysfs/gpu_min_clock\"' % (int(self.GPU_Max)) )			
			print (ADB + 'shell \"echo %d > /sys/devices/platform/gpusysfs/gpu_min_clock\"' % (int(self.GPU_Max)) )
			system(ADB + 'shell cat /sys/devices/platform/gpusysfs/gpu_min_clock')

			system(ADB + 'shell \"chmod 777 /sys/devices/platform/gpusysfs/gpu_max_clock\"'  )
			system(ADB + 'shell \"echo %d > /sys/devices/platform/gpusysfs/gpu_max_clock\"' % (int(self.GPU_Max)) )
			print(ADB + 'shell \"echo %d > /sys/devices/platform/gpusysfs/gpu_max_clock\"' % (int(self.GPU_Max)) )
			system(ADB + 'shell cat /sys/devices/platform/gpusysfs/gpu_max_clock')"""
			
			#system(ADB + 'shell \"chmod 444 /sys/devices/platform/gpusysfs/gpu_min_clock\"'  )
			#system(ADB + 'shell \"chmod 444 /sys/devices/platform/gpusysfs/gpu_max_clock\"'  )
		...
		# QDial Text
		self.label_little_min.setText(self.Little_Max) 
		self.label_little_max.setText(self.Little_Max)
		self.label_big_min.setText(self.Big_Max)
		self.label_big_max.setText(self.Big_Max) 
		self.label_gpu_min.setText(self.GPU_Max) 
		self.label_gpu_max.setText(self.GPU_Max)

		# QDial Value
		# QDial Value
		self.dial_Little.setValue( int(self.Little_Max) )
		self.dial_Big.setValue( int(self.Big_Max) )
		self.dial_GPU.setValue( int(self.GPU_Max) )
	...
	def ActionNormalMode(self):
		...
		if(DEVICE_CHIP == 'universal7420'):		#N5
			...
			"""system(ADB + 'shell \"chmod 777 /sys/devices/platform/gpusysfs/gpu_min_clock\"'  )
			system(ADB + 'shell \"echo %d > /sys/devices/platform/gpusysfs/gpu_min_clock\"' % (int(self.GPU_Min)) )
			#system(ADB + 'shell \"chmod 444 /sys/devices/platform/gpusysfs/gpu_min_clock\"'  )
...
#		system(ADB + 'pull /sdcard/recording.mp4 ' + str(filename) + '.mp4')
	...
	def DialLittlePress(self):
		print ('Press')
	
	def DialLittleRelease(self):
		print ('Release')
		pass
